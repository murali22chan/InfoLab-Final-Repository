...
def generate_data_v2():
    df = pd.read_csv("../data/open_qa_full.csv")


    # df = df.loc[:199]

    if os.path.exists("open_qa_full_truncated.csv"):
        print("Exisiting File Found")
        new_df = pd.read_csv("open_qa_full_truncated.csv")

    else:
        print("Truncating Texts")
        df['answer'] = df['answer'].apply(truncate_and_count_tokens)
        df.to_csv('open_qa_full_truncated.csv', index=False)
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            ...
            for perturbation_mode in ['d', 'z']:
                ...
                with open(os.path.join(SAVE_FOLDER, f"perturbation_{n_perturbations}_{perturbation_mode}_results_open_qa.json"),
                          "w") as f:
                    json.dump(output, f)
