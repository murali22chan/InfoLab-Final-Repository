...
if __name__ == '__main__':
	...
	parser.add_argument('--trainDomain1', type=str, default="finance")
	parser.add_argument('--trainDomain2', type=str, default="medicine")
	...
	wandb.init(project="domain-adaptation-combination-trial", config=args)

	print("Experiment Details")
	print("Training Data: "+str(args.trainDomain1)+", "+str(args.trainDomain2)+ " Testing Data: "+str(args.testDomain)+" Training Sequnce Length: "+str(args.trainSeqLength)+" Testing Sequence Length: "+str(args.testSeqLength))


	#Loading the domain csv
	train_df_1 = pd.read_csv(args.trainDomain1 + "_full.csv")
	train_df_2 = pd.read_csv(args.trainDomain2 + "_full.csv")
	cross_domain_df = pd.read_csv(args.testDomain + "_full.csv")

	train_text_1, train_labels_1, val_text_1, val_labels_1, _, _ = split_data_training_validation_testing_same_domain(train_df_1)
	train_text_2, train_labels_2, val_text_2, val_labels_2, _, _ = split_data_training_validation_testing_same_domain(train_df_2)

	train_text = pd.concat([train_text_1, train_text_2], axis=0)
	train_labels = pd.concat([train_labels_1, train_labels_2], axis=0)

	val_text = pd.concat([val_text_1, val_text_2], axis=0)
	val_labels = pd.concat([val_labels_1, val_labels_2], axis=0)
	...
	wandb.run.name = "bert_train_"+str(args.trainDomain1)+"_"+str(args.trainDomain2)+"_test_"+str(args.testDomain)+"_trainSeqLen_"+str(trainSequenceLength)+"_testSeqLen_"+str(testSequenceLength)
	...
	for epoch in tqdm.tqdm(range(epochs)):
		...
		if valid_accuracy > best_valid_acc:
			...
			model_name = "bert_train_"+str(args.trainDomain1)+"_"+str(args.trainDomain2)+"_test_"+str(args.testDomain)+"_trainSeqLen_"+str(trainSequenceLength)+"_testSeqLen_"+str(testSequenceLength)+".pt"
	...
	results = {"training_data1": args.trainDomain1, "training_data2": args.trainDomain2, "testing_data": args.testDomain,"training_seq_len": args.trainSeqLength,"testing_seq_len": args.testSeqLength,"accuracy": accuracy, "precision": macro_precision, "recall": macro_recall,"f1-score": macro_f1}
