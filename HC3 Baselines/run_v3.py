...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainDomain', type=str, default="reddit_eli5")
    ...
    train_domain = args.trainDomain

    if train_domain == "finance":
        epochs = 5
        lr = 0.00001
    if train_domain == "medicine":
        epochs = 10
        lr = 0.00001

    if train_domain == "wiki_csai":
        epochs = 20
        lr = 0.000005

    if train_domain == "open_qa":
        epochs = 10
        lr = 0.000002

    if train_domain == "reddit_eli5":
        epochs = 15
        lr = 0.000001


    # log the experiment settings
    wandb.init(project="hc3-baseline-roberta-newhps-itr1", config=args)
    ...
    wandb.run.name = "roberta_train_" + str(args.trainDomain) + "_test_" + str(args.testDomain) + "_trainSeqLen_" + str(
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        if valid_accuracy > best_valid_acc:
            ...
            model_name = "final_roberta_train_" + str(args.trainDomain) + "_test_" + str(
    ...
    best_model_path = "models/final_roberta_train_" + str(args.trainDomain) + "_test_" + str(
                args.testDomain) + "_trainSeqLen_" + str(trainSequenceLength) + "_testSeqLen_" + str(
                testSequenceLength) + ".pt"
