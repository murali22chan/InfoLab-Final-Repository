...
def get_roc_metrics(real_preds, sample_preds):
    fpr, tpr, _ = roc_curve([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds, pos_label=1)
    roc_auc = auc(fpr, tpr)
...
def get_precision_recall_metrics(real_preds, sample_preds):
    precision, recall, _ = precision_recall_curve([0] * len(real_preds) + [1] * len(sample_preds),
                                                  real_preds + sample_preds, pos_label=1)
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    smaller_length = min(original_length, sampled_length)
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    # Replace NaN, np.inf, and -np.inf values with 100
    for key in predictions:
        predictions[key] = [100 if np.isnan(x) or np.isinf(x) else x for x in predictions[key]]
...
def load_base_model_and_tokenizer(name):
    ...
    base_tokenizer = transformers.AutoTokenizer.from_pretrained(name, **optional_tok_kwargs, truncation = True, cache_dir=cache_dir)
...
def eval_supervised(data, model):
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(model, truncation = True, cache_dir=cache_dir)
...
def truncate_and_count_tokens(text, max_tokens=512):
...
def generate_data_v2():
    df = pd.read_csv("../data/reddit_eli5_full.csv")


    # df = df.loc[:199]

    if os.path.exists("reddit_eli5_full_truncated.csv"):
        print("Exisiting File Found")
        new_df = pd.read_csv("reddit_eli5_full_truncated.csv")

    else:
        print("Truncating Texts")
        df['answer'] = df['answer'].apply(truncate_and_count_tokens)
        df.to_csv('reddit_eli5_full_truncated.csv', index=False)
        # Filter rows where num_tokens is greater than 512
        new_df = df
    ...
    for index, row in new_df.iterrows():
        ...
        if label == 0:
            dict_data['original'].append(text)
        else:
            dict_data['sampled'].append(text)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip_baselines', action='store_false')
    parser.add_argument('--buffer_size', type=int, default=1)
    parser.add_argument('--mask_top_p', type=float, default=1.0)
    parser.add_argument('--pre_perturb_pct', type=float, default=0.0)
    parser.add_argument('--pre_perturb_span_length', type=int, default=5)
    parser.add_argument('--random_fills', action='store_false')
    parser.add_argument('--random_fills_tokens', action='store_false')
    ...
    GPT2_TOKENIZER = transformers.GPT2Tokenizer.from_pretrained('gpt2', truncation=True,  cache_dir=cache_dir)
    ...
    if not args.baselines_only and not args.random_fills:
        ...
        n_positions = 512
    preproc_tokenizer = transformers.AutoTokenizer.from_pretrained('t5-small', truncation=True, model_max_length=512,
                                                                   cache_dir=cache_dir)
    mask_tokenizer = transformers.AutoTokenizer.from_pretrained(mask_filling_model_name, truncation = True, model_max_length=n_positions,
                                                                cache_dir=cache_dir)
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            ...
            for perturbation_mode in ['d', 'z']:
                ...
                with open(os.path.join(SAVE_FOLDER, f"perturbation_{n_perturbations}_{perturbation_mode}_results_reddit_eli5.json"),
                          "w") as f:
                    json.dump(output, f)
